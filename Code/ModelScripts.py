...
def brute_force_mod(model, mod, div, hrs, group, sigma, dur, delay, noise):
    ...
    try:
        with tqdm(total=tot, file=sys.stdout) as pbar:
            # for i in np.linspace(model.g_Roff2R + (5/mod), model.g_Roff2R - (5/mod), div): # g_Roff2R
            #     for j in np.linspace(model.g_R2Roff + (5/mod), model.g_R2Roff - (5/mod), div): # g_R2Roff
...
def funnel_solve(div, hrs, group, sigma, dur, delay, noise, csvfile, mType):
    ...
    Args:
        ...
        noise {bool} -- adds noise to simulation if true 
        csvfile (ParamCSVFile): csv file to write data to (class from FileHandling.py)
        mType (string): 'MC' or 'CC' - declares model type
    ...
    for i in range(numIter):  
        if not inter:
            ...
            sc, mo, inter = brute_force_mod(model, mod, div, hrs, group, sigma, dur, delay, noise)
            ...
            if not None in m_arr:
                ...
                print('Winner:')
                print(f'    g_Roff2R: {model.g_Roff2R}')
                print(f'    g_R2Roff: {model.g_R2Roff}')
...
# create file for CC object
filename = 'paramSetsCC.csv'
paramSetsCC = None
for _, _, files in os.walk('.', topdown=False):
    if filename in files:
        paramSetsCC = ParamCSVFile(filename, False)
    else:
        paramSetsCC = ParamCSVFile(filename, True)
...
# print('----- Funnel-Solve -----')
# scores2, models2 = funnel_solve(div, hrs, group, sigma, dur, delay, noise, paramSetsMC, 'MC')

# mCC = ModelConcChanges(X0, dt)
# mCC.run_mi_model(hrs, group=group, sigma=sigma, dur=dur, delay=delay, noise=noise)
# mCC.hypnogram(p=1)
# s1 = score_model(mCC, pr=1, p=1)
...
# mMC.run_mi_model(hrs, group=group, sigma=sigma, dur=dur, delay=delay, noise=noise)
# mMC.hypnogram(p=1)
...
# X0 = [F_R, F_Roff, F_S, F_W, C_R, C_Rf, C_Roff, C_S, C_W, stp, h, delta, omega, sigma]
# IC = [5.0, 0.0, 4.0, 1.0, 0.2, 1.0, 0.0, 1.0, 0.4, 1.0, 1.0, 1.0, 1.0, 0]

# mOld = ModelOriginal(IC, dt)
# mOld.run_mi_model(hrs, group=group, sigma=sigma, dur=dur, delay=delay, noise=noise)
# mOld.hypnogram(p=1)
