...
class ModelConcChanges():
    ...
    def run_mi_model(self, hrs, group='None', sigma=0, dur=5*60, delay=0, gap=15*60, gap_rand=False, gap_range=[1, 25], noise=False):
        ...
        def mi_model_noise(simX, group, t = 0):
            ...
            # steady state for neurotransmitter concentrations:
            ...
            dF_R = (R_inf(CRoff_inf(F_Roff) * self.g_Roff2R + CW_inf(F_W) * self.g_W2R + sigma_R) - F_R) / self.tau_R
            ...
            dF_Roff = (Roff_inf(CR_inf(F_R) * self.g_R2Roff + CW_inf(F_W) * self.g_W2Roff + \
                delta + sigma_Roff) - F_Roff) / self.tau_Roff
            ...
            # firing rate of REM (R) population
            dF_W = (W_inf(CS_inf(F_S) * self.g_S2W + CR_inf(F_R) * self.g_R2W +
                        CRoff_inf(F_Roff) * self.g_Roff2W + 0.6*delta + sigma_W) - F_W) / self.tau_W
            # dynamics for neurotransmitter
            ...
            # firing rate of REM (R) population
            dF_S = (S_inf(CW_inf(F_W) * self.g_W2S + CRoff_inf(F_Roff) * self.g_Roff2S + sigma_S) - F_S) / self.tau_S
            # dynamics for neurotransmitter
