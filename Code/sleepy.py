...
def write_remidx(M, K, ppath, name, mode=1) :
    ...
    f = open(outfile, 'w')
    s = ["%d\t%d\n" % (i,j) for (i,j) in zip(M,K)] # zip(M[0,:],K)
    f.writelines(s)
    f.close()
...
#                    plt.legend(bbox_to_anchor=(1.0, 0.7, 1., .102), loc=1, mode='expand', ncol=nmice, frameon=False)
                    ax.legend(mouse_order, bbox_to_anchor=(0., 1.0, 1., .102), loc=3, ncol = int(len(mouse_order)/2), frameon = False)
                box_off(ax)
...
#        midx = np.arange(istart,iend+1)
    ...
    if pplot:
        ...
        plt.bar([1,2,3], PercMx.mean(axis=0), align='center', color='gray', fill=False, edgecolor='black')
        ...
        plt.bar([1,2,3], DurMx.mean(axis=0), align='center', color='gray', fill=False, edgecolor='black')
        plt.xticks([1,2,3], ['REM', 'Wake', 'NREM'], rotation=60)
        for i in range(nmice):
        ...
        plt.bar([1,2,3], FreqMx.mean(axis=0), align='center', color='gray', fill=False, edgecolor='black')
        plt.xticks([1,2,3], ['REM', 'Wake', 'NREM'], rotation=60)
        for i in range(nmice):
...
def sleep_spectrum(ppath, recordings, istate=1, pmode=1, fres=1/3, ma_thr=20.0, f_max=-1, pplot=True, sig_type='EEG', mu=[10, 100],
    ...
    if pplot:
        ...
        if sig_type == 'EEG':
            ...
            if pmode>=1:
                ax = plt.axes([0.2, 0.15, 0.3, 0.7])
                ax.bar([0], Ampl[0].mean(), color='gray', alpha = 0.5)
                ax.bar([1], Ampl[1].mean(), color='blue', alpha = 0.5)                
                for i in range(nmice):
                ...
                plt.ylabel('EMG Ampl. ($\mathrm{\mu V}$)')
                ax.set_xticks([0,1])
                ax.set_xticklabels(['Laser OFF', 'Laser ON'])
                plt.legend(bbox_to_anchor=(1, 0), loc=3)

                # some basic stats
                [tstats, p] = stats.ttest_rel(Ampl[0], Ampl[1])
                print("Stats for EMG amplitude: t-statistics: %.9f, p-value: %.9f" % (tstats, p))
...
def sleep_spectrum_simple(ppath, recordings, istate=1, tstart=0, tend=-1, fmax=-1, mu=[10,100], ci='sd', 
    ...
    if pmode == 1:
        ...
        list_lsr = [1]*len(freq)*len(mice) + [0]*len(freq)*len(mice)
...
#        df_amp['Lsr'] = ['no'] * len(mice) 
    ...
    if pplot:
        ...
        plt.figure()
        sns.set_style('ticks')
        sns.lineplot(data=df, x='Freq', y='Pow', hue='Lsr', ci=ci, palette={1:'blue', 0:'gray'})
#        sns.lineplot(data=df, x='Freq', y='Pow', hue='Lsr', ci=ci, palette={'yes':'blue', 'no':'gray'})
        ...
        plt.figure()
        plt.axes([0.1, 0.1, 0.3, 0.8])
        sns.barplot(data=df_amp, x='Lsr', y='Amp', palette={1:'blue', 0:'gray'}, ci=ci, alpha = 0.5)
#        sns.barplot(data=df_amp, x='Lsr', y='Amp', palette={'yes':'blue', 'no':'gray'}, ci=ci, alpha = 0.5)
