"""Flip-Flop Model with architectural changes implemented to F_W -> stp inhibition (stp decreases during wake)
...
class ModelWeber():
    """Object for Flip-Flop Model with architectural changes implemented to F_W -> stp inhibition (stp decreases during wake)
    """
    # Parameters
    R_max = 5.0 # 5.0
    Roff_max = 5.0 # 5.0
    ...
    def run_mi_model(self, hrs, group = 'None', sigma = 0, dur = 5*60, delay = 0, gap = 15*60, gap_rand = False, gap_range = [1, 25], noise = False):
        ...
        def mi_model_noise(simX, group, t = 0):
            ...
            [F_R, F_Roff, F_S, F_W, C_R, C_Rf, C_Roff,
                C_S, C_W, stp, h, delta, omega, sigma] = simX
            ...
            dC_R = (CR_inf(F_R) - C_R) / self.tau_CR
            ...
            dF_Roff = (Roff_inf(C_R * self.g_R2Roff + C_W * self.g_W2Roff + delta + sigma_Roff) - F_Roff) / self.tau_Roff

            def CRoff_inf(x): return CX_inf(x, self.gamma_Roff)
            dC_Roff = (CRoff_inf(F_Roff) - C_Roff) / self.tau_CRoff
            ...
            # dynamics for neurotransmitter
            dC_W = (CW_inf(F_W) - C_W) / self.tau_CW
            ...
            # dynamics for neurotransmitter
            dC_S = (CS_inf(F_S) - C_S) / self.tau_CS
    ...
    def inter_REM(self, p=0, zoom_out=0, nremOnly = False, log = False):
        ...
        if log:
            log_inter = np.log(inter_durations)
            #regression line and r^2
            logM, logB, logR, _, _ = stats.linregress(REM_durations, log_inter)
        ...
        #regression line and r^2
        m, b, r, _, _ = stats.linregress(REM_durations, inter_durations)
        ...
        if log and p == 1:
            ...
            plt.figure()
            sns.distplot(log_inter, hist = True, vertical=True)
