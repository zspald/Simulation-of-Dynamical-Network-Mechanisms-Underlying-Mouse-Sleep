...
mMCCV.load_model_data('mMCCV_80hr')
...
# ron_rem, ron_wake, ron_nrem, roff_rem, roff_wake, roff_nrem = mMCCV.avsg_Ron_and_Roff_by_state()
...
t, inter3_fRon, inter3_fRoff = mMCCV.hysteresis_traj(ind_shift=50, save_fig=True, filename='hysteresis_traj_long_6')
...
mTest = ModelTest(IC_conc_var_steady_state, dt)
# mTest.g_Roff2R = -7.0
# mTest.g_R2Roff = -5.0
...
mTest.run_mi_model(24 + 2, group=group, sigma=sigma, dur=dur, delay=delay, gap=gap, noise=noise, refractory_activation=False)
mTest.hypnogram_fig1(p=1, save=False, filename='no_noise_hysteresis_test_hypno')
